...
function LecturerDashboard() {
  ...
  const qrWsRef = useRef(null);  // holds the live QR WebSocket so we can close it cleanly
      ...
      const fetchLecturerData = async () => {
          ...
          try {
              const [crsRes, ttRes, notifRes, profRes] = await Promise.all([
                  fetch(`${import.meta.env.VITE_API_URL}/api/v1/users/me/lecturer/courses`, { headers: { Authorization: `Bearer ${token}` } }),
                  fetch(`${import.meta.env.VITE_API_URL}/api/v1/users/me/lecturer/timetable`, { headers: { Authorization: `Bearer ${token}` } }),
                  fetch(`${import.meta.env.VITE_API_URL}/api/v1/users/me/notifications`, { headers: { Authorization: `Bearer ${token}` } }),
                  fetch(`${import.meta.env.VITE_API_URL}/api/v1/users/me/profile`, { headers: { Authorization: `Bearer ${token}` } })
              ]);
  ...
  // ── QR WebSocket ───────────────────────────────────────────────────
  // Replaces the HTTP polling pattern (fetchQrToken every 10s).
  // The backend pushes a new qr_token over the socket every 10 seconds —
  // no browser timer needed, no wasted HTTP round-trips.
  const openQrSocket = (id) => {
    // Close any stale socket first (e.g. if start is clicked twice)
    ...
    const ws = new WebSocket(`${wsUrl}/api/v1/academic/ws/session/${id}/qr`);

    ws.onmessage = (e) => {
      try {
        const { qr_token } = JSON.parse(e.data);
        setQrToken(qr_token);
        localStorage.setItem("current_qr_token", qr_token);
        setQrSeed(s => s + 1);
      } catch (err) {
        console.error("QR WebSocket parse error", err);
      }
    };

    ws.onerror = (e) => console.error("QR WebSocket error", e);

    ws.onclose = () => console.log("QR WebSocket closed");
        ...
        async (position) => {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    setSecondsSinceStart(0);
                    openQrSocket(data.session_id);  // open WS — backend pushes QR tokens
  ...
  const handleStopClass = async () => { 
    ...
    // Close the QR WebSocket cleanly
    if (qrWsRef.current) {
      qrWsRef.current.close();
      qrWsRef.current = null;
    }
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setSecondsSinceStart(prev => {
          ...
          if (next >= 600) { 
              setSessionActive(false);
              // Close QR socket on timeout
              if (qrWsRef.current) { qrWsRef.current.close(); qrWsRef.current = null; }
              showToast("Session timed out (10 min limit reached).", "error"); 
              return 0; 
          }
          // QR rotation is now handled by the WebSocket — no fetchQrToken needed here
          return next;
      });
    }, 1000);
